...
5. 자소서 첨삭 완료 시 `status=completed`, 화면에 **적합도 게이지 + 문항별 점수/개선 포인트/개선안** 렌더링.
...
- **공고 크롤링 + 이미지 공고 대응**: 텍스트 공고는 파싱, JD가 이미지 한 장인 공고는 스크린샷 → 비전으로 읽음.
...
- **향후 개선**
  - 분석 피드백을 더 구체적으로(부족한 점 상세화) — 프롬프트 고도화
  - `submissions.status` 기반 재처리/재시도, Kafka healthcheck
  - 자소서 첨삭 few-shot 예시, 결과 히스토리 저장/비교
